...
- **Frontend:** Server-rendered HTML, one small vanilla JavaScript module, and project-owned CSS. Do not load external CSS or JavaScript. Avoid a frontend framework and build pipeline until the interaction model proves it needs one.
- **Markdown:** Store raw Markdown only in descriptions and comments. Render it on the server with a maintained parser, enable only the required extensions, and sanitize/allowlist the resulting HTML before serving it. Summaries remain plain text. Do not implement a Markdown parser or HTML sanitizer from scratch.
...
- status (`open`, `resolved`, or `no_action`); duplicate context can remain in the description/comments
...
The `/transactions` page is a read-only history explorer. The first cut searches transaction summary/actor and filters by state or kind, shows undo/redo relationships and retained abandoned branches, and links affected object IDs when available. It is a query/presentation layer over durable transaction records, not a replacement for the primary stream view.

The application also exposes a compact logical history-position summary for the primary view. This is based on transaction relationships rather than stream `updated_at`: an undo may update object revisions now while leaving the visible data at an older transaction position. The summary identifies the original transaction actor/time and whether the workspace is at the current transaction head or viewing an older position.
...
Use resource-oriented endpoints with explicit version or revision preconditions on mutating stream/comment operations. A write that supplies an old revision should return a conflict response and the current representation, rather than overwriting it. Deep links should use stable identifiers and encode rooted-view, focus, search query, and view/sort state in a bookmarkable form. Search syntax, matching, rooted scope, and minimum ancestor expansion are defined in [Search](search.md). Shareable URL state is separate from client-local presentation preferences such as expansion and scroll position. Client-local hierarchy state is keyed by bundle and root, not sort mode; restoring a target expands only the minimum ancestor chain needed to reveal it.

The initial JSON API exposes bundle listing/creation, bundle stream listing/creation, stream read/update/delete/status, and comment listing/creation/update/delete under `/api/`. Stream reads and bundle stream listings include comments for the first UI slice. Responses may expose raw Markdown fields for editing and separately rendered, sanitized HTML fields for display; summaries are plain text and are escaped rather than Markdown-rendered. Mutating stream/comment requests include an `actor` display name; stream updates, status changes, and deletes include revision preconditions. Bulk status changes check all selected stream revisions and update them atomically. Stream owners and tags are stored as ordered lists of trimmed, non-empty strings, and deadlines use the date-only `YYYY-MM-DD` representation (empty values become null); input also accepts `YYYY/MM/DD` and normalizes it to hyphens. The UI displays date-only values as `YYYY-MM-DD` and renders timestamps with the same date ordering while retaining their time and timezone, such as `YYYY-MM-DD HH:MM:SSZ`. The modal converts comma/whitespace-separated tag input to that list before sending it. Favorite changes are revision-checked stream updates and are shared workspace state. A single-stream delete removes the complete subtree and all descendant comments by default; the explicit delete mode `promote` deletes only the focused stream and its comments while promoting direct children one level and preserving their subtrees. Delete operations retain before snapshots in history and return HTTP 409 with the current object when its revision is stale.
...
The block-delete endpoint accepts contiguous sibling root IDs and revisions for every stream in their subtrees. It deletes the selected roots, descendants, and comments in one transaction, records before snapshots, and rejects the entire request if any affected stream is stale.
...
External-reference recognition, built-in stream/comment links, and declarative plugin rendering are described in [Integrations](integrations.md). Markdown rendering must use an allowlisted/sanitized renderer. Rendering and reference substitution operate on text nodes and must not rewrite existing links or code.
