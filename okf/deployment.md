...
The command checks SQLite's structural and foreign-key integrity plus application-level invariants that SQLite cannot express, including stream parent cycles, cross-bundle parents, duplicate sibling order keys, malformed JSON fields, malformed history records, and dangling transaction references. Stream-related diagnostics include the stream summary, full ID, bundle, and immediate parent context (without printing stream Markdown bodies), so corrupted records can be identified from the report. Historical records for deleted objects are valid and are retained. It exits `0` when the database is clean and nonzero when it finds issues. It never repairs or writes the database. Use `--database PATH` or `STREAM_DATABASE_PATH` to select another datastore.
