...
- Descriptions and comments show rendered Markdown when idle; summaries remain plain text. Rendered body content has a subtle boundary and background treatment so it is distinguishable from surrounding metadata without an extra label. Existing description/comment modals open in rendered mode with a clear Edit toggle that switches to the original Markdown source; Save remains available after switching back to rendered view so a changed draft can be saved without another mode change. New body content opens in Edit mode. Saving returns to rendered content or closes according to the existing modal flow.
