...
- The filter/search field follows the grammar and rooted-result behavior in [Search](search.md). Search results are flat within the active view, retain relative breadcrumbs, and expand only the minimum ancestor paths needed to reveal matching streams; unrelated branches remain collapsed.
- The header breadcrumbs show the full rooted path, such as `Index / parent / effort1`, even when the target was opened through a deep link or a one-shot jump. Index and ancestor breadcrumbs are clickable and re-root the current view at that level; the current stream is a non-interactive label with `aria-current="page"`. The active view/sort (including Deadline) is preserved. The Index/root view keeps the existing bundle name display. The viewbar’s open/total stream counts cover the current rooted subtree and intentionally do not change when the search filter narrows the displayed rows.
...
- Hierarchy uses three coordinated visual cues in tree views: the existing depth indentation, a depth-aligned vertical rail, and a neutral background tint that begins at that rail, extends to the end of the row, and increases clearly with depth. The area to the left of the rail remains untinted. Focus/selection always takes precedence with a strong blue outline and leading accent; move selection retains its amber treatment. Deadline and other flat views do not use depth tinting or rails and continue to show hierarchy through their existing breadcrumbs.
...
- Descriptions and comments show rendered Markdown when idle; summaries remain plain text. Rendered body content has a subtle boundary and background treatment so it is distinguishable from surrounding metadata without an extra label. Existing description/comment modals open in rendered mode with Copy ref, Copy link, and Edit controls in that order; Edit switches to the original Markdown source, and Save remains available after switching back to rendered view so a changed draft can be saved without another mode change. Stream summaries and metadata remain editable in the default modal mode. There is no heading close control; Cancel and Escape close the dialog. New body content opens in Edit mode. Saving returns to rendered content or closes according to the existing modal flow.
...
- Each stream row has a compact pencil affordance at the end of its summary line for opening the stream modal. The existing-stream modal opens with the summary and metadata fields editable, while the description is rendered until Edit is pressed; Edit reveals the original Markdown description source. Its heading actions are ordered Copy ref, Copy link, Edit, with Edit receiving focus, and there is no redundant heading close button because Cancel and Escape close the dialog. The modal supports full-width summary and description fields, a compact inline metadata row for comma-separated owners, numeric priority, and an accessible date-only deadline input, followed by a compact Tags row. Tags accept commas or whitespace as separators and preserve the entered order after empty values are removed. Owners and tags have no suggested names; both may be empty. Required fields use a trailing `*` in their visible label while retaining semantic required attributes; the summary is currently the only required field. Create and edit use the same fields.
...
- Render descriptions and comments as server-generated, sanitized Markdown HTML with safe ordinary links and declarative internal/external references. Escape summaries as plain text. Existing comment modals follow the same rendered-first pattern: Copy ref, Copy link, Edit (focused), no heading close button, and Edit reveals the Markdown source while Cancel/Escape remain available.
...
- Deletion is a confirmed action: `d` shows a pending command state and `d s`/`d S`/`d c` open the single-object confirmation modals. `d v` opens a confirmation for a completed visual selection; it reports selected roots, descendant count, and comments, then deletes the complete selected subtrees atomically without promoting children. `d c` acts only on the exact comment under focus (`commentIndex`), never implicitly on the newest comment, and is a no-op when stream content has focus. `d s` acts on the containing/focused stream even when the comment rail has focus and deletes its complete subtree. `d S` deletes only the focused stream and promotes its direct children one level while preserving their descendants. The stream modals use distinct titles, descriptions, and submit labels that state whether descendants are deleted or children promoted. A stale delete leaves the modal open with a conflict message.
