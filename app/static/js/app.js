...
(() => {
  ...
  function displayChildrenOf(id, items) {
    if (id !== null) return childrenOf(id, items);
    return [...childrenOf(null, items), ...items.filter((stream) => stream.parent_stream_id === stream.id)];
  }
  ...
  function navigationEntries() {
    ...
    const rootId = currentRootId();
    ordered(rootId ? items.filter((stream) => stream.id === rootId) : displayChildrenOf(null, items)).forEach((stream) => visit(stream, 0));
    return result;
  }
  ...
  function focusCurrentTarget() {
    ...
    const row = document.querySelector(`[data-id="${CSS.escape(state.selected)}"]`);
    const target = state.focusColumn === "comments" ? row?.querySelector(`[data-comment-index="${state.commentIndex}"]`) : row;
    (target || row)?.focus();
    revealInViewport(target || row);
  }
  function restoreCommentFocus() { if (state.focusColumn !== "comments" || !state.selected) return; const stream = selectedStream(); const index = state.commentId ? stream?.comments?.findIndex((comment) => comment.id === state.commentId) ?? -1 : state.commentIndex; if (index >= 0) state.commentIndex = index; focusCurrentTarget(); }
  ...
  function moveVertical(direction) { const items = navigationItems(); if (!items.length) return; const index = items.findIndex((stream) => stream.id === state.selected); const start = index < 0 ? (direction > 0 ? -1 : items.length) : index; const target = items[Math.max(0, Math.min(items.length - 1, start + direction))]; if (target) { const preserve = state.focusColumn === "comments" && (target.comments || []).length; state.commentIndex = preserve ? Math.min(state.commentIndex, target.comments.length - 1) : 0; state.commentId = preserve ? target.comments[state.commentIndex]?.id || null : null; state.selected = target.id; if (state.selecting) updateSelection(target.id); else if (!state.selectionReady) { state.selectedIds = []; select(target.id, Boolean(preserve)); } if (state.moveMode) state.moveMode.targetId = target.id; render(); savePresentationAndUrl(); focusCurrentTarget(); } }
  ...
  function moveHorizontal(direction) { const stream = selectedStream(); if (!stream) return; const count = (stream.comments || []).length; if (direction > 0 && state.focusColumn === "stream" && count) { state.focusColumn = "comments"; state.commentIndex = 0; state.commentId = stream.comments[0]?.id || null; } else if (direction > 0 && state.focusColumn === "comments") { state.commentIndex = Math.min(state.commentIndex + 1, count - 1); state.commentId = stream.comments[state.commentIndex]?.id || null; } else if (direction < 0 && state.focusColumn === "comments" && state.commentIndex > 0) { state.commentIndex -= 1; state.commentId = stream.comments[state.commentIndex]?.id || null; } else if (direction < 0) { state.focusColumn = "stream"; state.commentId = null; } render(); savePresentationAndUrl(); focusCurrentTarget(); }
