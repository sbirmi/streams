(() => {
  ...
  function moveVertical(direction) { const items = navigationItems(); if (!items.length) return; const index = items.findIndex((stream) => stream.id === state.selected); const start = index < 0 ? (direction > 0 ? -1 : items.length) : index; const target = items[Math.max(0, Math.min(items.length - 1, start + direction))]; if (target) { const preserve = state.focusColumn === "comments" && (target.comments || []).length; state.commentIndex = preserve ? Math.min(state.commentIndex, target.comments.length - 1) : 0; state.commentId = preserve ? target.comments[state.commentIndex]?.id || null : null; state.selected = target.id; if (state.selecting) updateSelection(target.id); else if (!state.selectionReady) { state.selectedIds = []; select(target.id, Boolean(preserve)); } if (state.moveMode) state.moveMode.targetId = target.id; render(); savePresentationAndUrl(); } }
