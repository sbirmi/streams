(() => {
  ...
  function setEditorMode(mode) { state.editorMode = mode; const rendered = document.querySelector("#editor-rendered"); const fields = document.querySelector("#editor-fields"); const toggle = document.querySelector("#editor-mode-toggle"); const stream = state.editing; const isRendered = mode === "rendered" && Boolean(stream); rendered.hidden = !isRendered; fields.hidden = isRendered; toggle.hidden = !stream; toggle.textContent = isRendered ? "Edit" : "View"; if (isRendered) { document.querySelector("#editor-rendered-summary").textContent = stream.summary || ""; document.querySelector("#editor-rendered-description").innerHTML = renderedBody(stream, "description", "description_html"); } }
  ...
  function setCommentMode(mode) { state.commentMode = mode; const rendered = document.querySelector("#comment-rendered"); const fields = document.querySelector("#comment-fields"); const toggle = document.querySelector("#comment-mode-toggle"); const comment = state.commentEditing; const isRendered = mode === "rendered" && Boolean(comment); rendered.hidden = !isRendered; fields.hidden = isRendered; toggle.hidden = !comment; toggle.textContent = isRendered ? "Edit" : "View"; if (isRendered) document.querySelector("#comment-rendered-body").innerHTML = renderedBody(comment, "body", "body_html"); }
