...
- [x] **Make insertion commands explicit and rooted-view safe.** Replaced single-key insertion with `ip`/`in`/`ic`, blocked invisible sibling/root mutations in rooted views, added rooted-context API validation, and documented legacy shortcut fallback behavior.
