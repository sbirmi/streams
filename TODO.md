...
- [x] **Implement documented search.** Added the `okf/search.md` source of truth, AND/negation and field parsing, comment matching, shareable `q` URL state, rooted-scope filtering, and minimum ancestor expansion. Verified with `./scripts/test` (59 tests passing) and `node --check`.
- [x] **Add a read-only datastore integrity audit.** Added `./scripts/check-data` for SQLite, tree, payload, history, and transaction checks, with temporary-database corruption tests and deployment documentation.
