(() => {
  ...
  function renderComments(stream) {
    ...
    return `<div class="comment-rail" aria-label="Comments">${stream.comments.map((comment) => `<article class="comment-card"><div class="comment-meta"><strong>${escapeHtml(comment.author)}</strong><span>${escapeHtml(comment.updated)}</span></div><div class="comment-body">${renderMarkdown(comment.text)}</div></article>`).join("")}</div>`;
  }
